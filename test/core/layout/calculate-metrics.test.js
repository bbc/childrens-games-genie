...
describe("Layout - Calculate Metrics", () => {
    ...
    describe("borderPad metric", () => {
        test("sets a vertical border padding of 2% of the longest edge when not an iPhone 5 (568x320)", () => {
            expect(getMetrics({ width: 600, height: 600 }).bottomBorderPad).toBe(16);
            expect(getMetrics({ width: 800, height: 600 }).bottomBorderPad).toBe(16);
            expect(getMetrics({ width: 1000, height: 600 }).bottomBorderPad).toBe(20);
            expect(getMetrics({ width: 1500, height: 600 }).bottomBorderPad).toBe(28);

            expect(getMetrics({ width: 200, height: 600 }).bottomBorderPad).toBe(16);
            expect(getMetrics({ width: 200, height: 800 }).bottomBorderPad).toBe(16);
            expect(getMetrics({ width: 200, height: 1000 }).bottomBorderPad).toBe(16);
        });

        test("sets a vertical border padding of 32 when it is a iPhone 5 (568x320)", () => {
            expect(getMetrics({ width: 568, height: 320 }).bottomBorderPad).toBe(52);
        });
